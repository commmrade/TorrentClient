...
QHash<int, QByteArray> FileTableModel::roleNames() const { return {}; }
...
void FileTableModel::setFiles(const QList<File> &files)
{
    ...
    for (auto &file : files)
    {
        qDebug() << file.id << file.isEnabled << file.filename << file.filesize << file.downloaded
                 << file.priority;
    }
    qDebug() << "======DEBUG=======";

    qDebug() << "Setting " << files.size();
    // TODO: Optimize it, since it is suitable for torrents list, but i guess i can do it easier for
    // (almost) static files
