...
class BaseItem
{
  public:
    BaseItem() = default;
    virtual ~BaseItem() { qDeleteAll(m_children); }
    ...
    QList<BaseItem *> children() const { return m_children; }
    void              removeChild(BaseItem *child) { m_children.remove(m_children.indexOf(child)); }
    int               childCount() const { return m_children.size(); }

    void      setParent(BaseItem *newParent);
    BaseItem *parent() const { return m_parent; }
...
class FileItem : public BaseItem
{
  ...
  public:
    ...
    void    setPath(const QString &newPath) override { m_fileData.filename = newPath; }
    QString getPath() const override { return m_fileData.filename; }

    void setData(const File &data) { m_fileData = data; }
    File getData() const { return m_fileData; }
...
class DirItem : public BaseItem
{
  ...
  public:
    DirItem() = default;
    void    setPath(const QString &newPath) override { m_path = newPath; }
...
class FileTreeModel : public QAbstractItemModel
{
    ...
    bool isRootVirgin() const { return (m_rootItem->childCount() == 0); }
