...
PiecesBarWidget::PiecesBarWidget(QWidget *parent) : QWidget(parent), ui(new Ui::PiecesBarWidget)
{
    ui->setupUi(this);
}

PiecesBarWidget::~PiecesBarWidget() { delete ui; }
...
void PiecesBarWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (m_pieces.empty())
    { // Draw empty background in case when there are no pieces to not waste resources
    ...
    if (piecesPerPixel) // That means if pieces > width available
    {
        ...
        for (int i = 0; i < chunks; ++i)
        {
            ...
            for (auto pidx = piecesPerPixel * i; pidx < pidxUntil; ++pidx)
            {
                ...
                {
                    auto iter = std::find_if(m_downloadingPiecesIndices.begin(),
                                             m_downloadingPiecesIndices.end(),
                                             [pidx](const auto value) { return value == pidx; });
        ...
        for (int i = 0; i < m_pieces.size(); ++i)
        {
            ...
            if (curPos >= endX - pixelsInPiece)
            {
                ...
                else
                {
                    auto iter = std::find_if(m_downloadingPiecesIndices.begin(),
                                             m_downloadingPiecesIndices.end(),
                                             [i](const auto value) { return value == i; });
                    if (iter != m_downloadingPiecesIndices.end())
                    {
