...
PeerTableModel::PeerTableModel(QObject *parent) : QAbstractTableModel{parent}
{
...
inline double ceilTwoAfterComa(double number) { return std::ceil(number * 100.0) / 100.0; }
...
QHash<int, QByteArray> PeerTableModel::roleNames() const { return {}; }
...
QString PeerTableModel::countryFromIp(QByteArrayView ip)
{
    ...
    if (m_mmdb.filename)
    {
        int                  gai_status;
        int                  status;
        MMDB_lookup_result_s search_result =
            MMDB_lookup_string(&m_mmdb, ip.data(), &gai_status, &status);
        if (status != MMDB_SUCCESS)
        {
        ...
        {
            countryName.assign(entry_data.utf8_string,
...
void PeerTableModel::setPeers(const std::vector<libtorrent::peer_info> &peers)
{
    ...
    auto conToStr = [](lt::connection_type_t t) -> QString
    {
        switch (t)
        {
        case lt::peer_info::standard_bittorrent:
            return "BT";
    ...
    auto makeKeyPeer = [](const Peer &peer) { return peer.ip + ":" + QString::number(peer.port); };
    auto makeKeyPeerInfo = [](const lt::peer_info &peer)
    {
        return QString::fromStdString(peer.ip.address().to_string()) + ":" +
               QString::number(peer.ip.port());
    };
    ...
    for (auto i = 0; i < peers.size(); ++i)
    {
        ...
        if (oldPeersMap.contains(newKey))
        {
            ...
            if (oldPeer.progress != progress || oldPeer.upSpeed != upSpeed ||
                oldPeer.downSpeed != downSpeed || oldPeer.downloaded != downloaded ||
        ...
        else
        {
            ...
            m_peers.append(Peer{countryName,
                                QString::fromStdString(peers[i].ip.address().to_string()),
                                peers[i].ip.port(), conToStr(peers[i].connection_type),
                                QString::fromStdString(peers[i].client), peers[i].progress,
    ...
    {
        QString const key = makeKeyPeer(m_peers[i]);
        if (!newPeersMap.contains(key))
        { // if peer exists in old, but doesnt in new, it is deleted
