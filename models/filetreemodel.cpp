...
bool FileTreeModel::setData(const QModelIndex &index, const QVariant &value, int role) {
    ...
    if (role == Qt::EditRole) {
        ...
        switch (static_cast<FileFields>(index.column())) { // TODO: Может сделать коннект с fileObj на statusChanged и priorityChanged и передавать в this::statusChanged, на данный момент это костыль
            case FileFields::STATUS: {
...
void FileTreeModel::traverseRecursively(QString curDir, BaseItem *root, const QList<File> &hFiles, const QModelIndex &parentIndex) {
    ...
    for (auto i = 0; i < fileChildrenSize; ++i) {
        ...
        if (fileChild->isDir()) {
            ...
            if (hFilesIter == hFiles.end()) { // The problem is that it throws when the current torrent is changed, therefore new set of files is passed, which does not contain old files
                // To fix it, i need to prematurily check if newly passed set of files is different from what is in the tree.
                // To do this, i might recursively collect all files in a tree but that is some zoomer level logic
                //
                // clearFiles();
                // setFiles(hFiles);
